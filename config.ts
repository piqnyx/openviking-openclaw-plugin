...
export type MemoryOpenVikingConfig = {
  ...
  apiKey?: string;
  /**
   * Path to the `agent = api key` file mapping OpenClaw agents to their own
   * OpenViking accounts. Agents missing from it — and traffic that cannot be
   * attributed to an agent at all — fall back to `apiKey` above.
   */
  ...
  /** Agent-visible remove_resource tool is disabled by default because it is destructive. */
...
const DEFAULT_AGENT_KEYS_FILE = "/home/openclaw/memory/secrets/openviking-keys/secrets.conf";
...
export const memoryOpenVikingConfigSchema = {
  parse(value: unknown): ParsedMemoryOpenVikingConfig {
    ...
    return {
      ...
      agentKeysFile: expandHomeDir(
        typeof cfg.agentKeysFile === "string" && cfg.agentKeysFile.trim()
          ? resolveEnvVars(cfg.agentKeysFile.trim())
          : getEnv("OPENVIKING_AGENT_KEYS_FILE")?.trim() || DEFAULT_AGENT_KEYS_FILE,
      ),
