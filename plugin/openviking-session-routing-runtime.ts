...
export function createOpenVikingSessionRoutingRuntime(options: {
  ...
  const unattributedSessionsWarned = new Set<string>();
   ...
   * system account.
   */
  const statedAgentId = (raw?: string): string | undefined => {
    const trimmed = typeof raw === "string" ? raw.trim() : "";
    if (!trimmed) {
      return undefined;
    }
    return sanitizeOpenVikingAgentIdHeader(
      configAgentPrefix ? `${configAgentPrefix}_${trimmed}` : trimmed,
    );
  };
   ...
   */
  const warnUnattributed = (sessionId?: string, sessionKey?: string, ovSessionId?: string) => {
    ...
    options.logger.warn(
      `openviking: no agent could be resolved for session ${marker} — routing to the system ` +
