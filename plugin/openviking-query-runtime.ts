...
export type OpenVikingQueryRuntimeDeps<TQueryConfigContext> = {
  getClient: (agentId: string | undefined) => Promise<OpenVikingQueryClient>;
  queryConfigStore: {
...
export function createOpenVikingQueryRuntime<TQueryConfigContext>(deps: OpenVikingQueryRuntimeDeps<TQueryConfigContext>): {
  ...
  const readOpenVikingContent = async (
    ...
    const client = await deps.getClient(accountAgentId);
  ...
  const multiReadOpenVikingContent = async (
    input: OpenVikingMultiReadInput,
    agentId?: string,
    accountAgentId?: string,
  ) => {
  ...
  const listOpenVikingDirectory = async (
    input: OpenVikingListInput,
    agentId?: string,
    accountAgentId?: string,
  ) => {
    const uri = input.uri.trim();
  ...
  const searchOpenViking = async (input: OpenVikingSearchInput, agentId?: string, traceCtx?: OpenVikingQuerySession) => {
    ...
    const client = await deps.getClient(traceCtx?.agentId);
