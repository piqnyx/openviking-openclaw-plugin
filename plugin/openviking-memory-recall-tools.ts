...
export type OpenVikingMemoryRecallToolsDeps = {
  ...
  getClient: (agentId: string | undefined) => Promise<OpenVikingMemoryRecallClient>;
  queryConfigStore: {
    getEffective: (
  ...
  deps.registerTool(
    (ctx: OpenVikingMemoryRecallToolContext) => ({
      ...
      async execute(_toolCallId: string, params: Record<string, unknown>) {
        ...
        const recallClient = await deps.getClient(session.agentId);
        ...
        if (targetUri) {
          ...
          const settled = await Promise.allSettled(searchPlan.searches.map((search) =>
            recallClient.find(
              query,
              {
                ...
                // Память отбираем только по записям: L0/L1 — описания
                // каталогов, на тематический запрос они обгоняют сами факты.
                // Ресурсы не трогаем, там уровни не фильтруются.
                level: search.resourceType === "resource" ? undefined : [2],
                actorPeerId: session.actorPeerId,
              },
            ),
          ));
          ...
          const leafOnly = uniqueMemories.filter((m) => m.level === undefined || m.level === 2);
          result = {
            memories: leafOnly,
        ...
        const leafOnly = (result.memories ?? []).filter((m) => m.level === undefined || m.level === 2);
