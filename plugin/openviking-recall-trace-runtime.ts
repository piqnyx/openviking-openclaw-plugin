...
export type OpenVikingRecallTraceRuntimeDeps = {
  getClient: (agentId: string | undefined) => Promise<{ read: (uri: string, agentId?: string) => Promise<unknown> }>;
...
export function createOpenVikingRecallTraceRuntime(deps: OpenVikingRecallTraceRuntimeDeps): {
  ...
  const queryRecallTraces = async (
    ...
    return enrichTraceEntriesWithContent(
      base,
      shouldIncludeTraceContent(input),
      session.actorPeerId,
      session.agentId,
    );
  };
