...
export type ResourceRoutingEmbeddingConfig = ResourceRoutingEndpointConfig & {
  dimensions?: number;
};
...
export type ParsedResourceRoutingEndpointConfig = {
  baseUrl: string;
  model: string;
  apiKey: string;
  headers: Record<string, string>;
...
export type ParsedResourceRoutingConfig = {
  ...
  reranker: ParsedResourceRoutingEndpointConfig;
  retrieval: {
    topK: number;
    minScore: number;
    rerankBelowMargin: number;
...
function expandEnv(value: string, label: string): string {
...
function parseFileTemplate(value: unknown, fallback: string, label: string): string {
...
function parseBaseUrl(value: unknown, fallback: string, label: string): string {
...
function parseSemanticInputTemplate(value: unknown): string {
  const template = stringValue(
    value,
    DEFAULT_SEMANTIC_INPUT_TEMPLATE,
    "openviking resourceRouting.semanticInputTemplate",
  );
  ...
  if (unknown.length > 0) {
    throw new Error(
      `openviking resourceRouting.semanticInputTemplate has unknown placeholders: ${unknown.join(", ")}`,
    );
  }
...
export function parseResourceRoutingConfig(value: unknown): ParsedResourceRoutingConfig {
  ...
  const reranker = parseEndpoint(
    cfg.reranker,
    { baseUrl: DEFAULT_RERANKER_BASE_URL, model: DEFAULT_RERANKER_MODEL },
    "openviking resourceRouting.reranker",
  );
  ...
  assertAllowedKeys(
    ...
    "openviking resourceRouting.retrieval",
  );

  const audit = toRecord(cfg.audit, "openviking resourceRouting.audit");
  assertAllowedKeys(
    audit,
    ["enabled", "file", "summaryPreviewChars"],
    "openviking resourceRouting.audit",
  );
  ...
  const fallbackCategory = stringValue(
    ...
    "openviking resourceRouting.fallbackCategory",
  );
  ...
  return {
    ...
    taxonomyFile: parseFileTemplate(
      cfg.taxonomyFile,
      DEFAULT_TAXONOMY_FILE,
      "openviking resourceRouting.taxonomyFile",
    ),
    cacheFile: parseFileTemplate(
      cfg.cacheFile,
      DEFAULT_CACHE_FILE,
    ...
    embedding: {
      ...
      dimensions: integerInRange(
        ...
        1,
        65_536,
        "openviking resourceRouting.embedding.dimensions",
      ),
    },
    reranker,
    retrieval: {
      topK: integerInRange(
        ...
        1,
        50,
        "openviking resourceRouting.retrieval.topK",
      ),
      minScore: numberInRange(
        retrieval.minScore,
        DEFAULT_MIN_SCORE,
        -1,
        1,
        "openviking resourceRouting.retrieval.minScore",
      ),
      rerankBelowMargin: numberInRange(
        ...
        0,
        2,
        "openviking resourceRouting.retrieval.rerankBelowMargin",
      ),
    },
    fallbackCategory,
    failurePolicy: "error",
    audit: {
      ...
      file: parseFileTemplate(
        audit.file,
        DEFAULT_AUDIT_FILE,
        "openviking resourceRouting.audit.file",
      ),
      summaryPreviewChars: integerInRange(
        audit.summaryPreviewChars,
        DEFAULT_AUDIT_SUMMARY_PREVIEW_CHARS,
        0,
        10_000,
        "openviking resourceRouting.audit.summaryPreviewChars",
      ),
    },
  };
}
