...
export type ResourceRoutingAuditTiming = {
  embeddingMs?: number;
  rerankerMs?: number;
  totalMs?: number;
};

export type ResourceRoutingAuditInput = {
  filePath: string;
  agentId: string;
  source: string;
  sourceKind?: string;
  ...
  timing?: ResourceRoutingAuditTiming;
...
export function writeResourceRoutingAudit(input: ResourceRoutingAuditInput): void {
  if (!input.filePath) {
