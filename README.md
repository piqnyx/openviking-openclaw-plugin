...
The fork keeps the upstream context-engine behavior while routing each OpenClaw agent to a separate OpenViking account/API key. Release **2026.7.15-isolation.8** adds configurable per-agent resource routing on top of the guarded `remove_resource` support introduced in `isolation.6`.
