...
async function readResponsePayload(response, label) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`${label} request failed with HTTP ${response.status}${compact ? `: ${compact}` : ""}`);
    }
    try {
...
async function postJson(url, body, config, label, transport) {
...
function parseEmbeddingResponse(payload, count, dimensions) {
    ...
    for (let position = 0; position < data.length; position += 1) {
        ...
        seen.add(index);
        vectors[index] = validateEmbeddingVector(raw.embedding, dimensions, `resource routing embedder data[${position}].embedding`);
    }
    if (seen.size !== count) {
...
function parseRerankRecord(record, position, count) {
    if (!record || typeof record !== "object" || Array.isArray(record)) {
...
function parseRerankResponse(payload, count) {
...
export class ResourceRoutingEmbeddingClient {
    ...
    async embed(texts) {
        ...
        const payload = await postJson(endpointUrl(this.#config.baseUrl, "embeddings"), {
            model: this.#config.model,
            input: normalized,
            encoding_format: "float",
        }, this.#config, "resource routing embedder", this.#transport);
...
export class ResourceRoutingRerankerClient {
    ...
    async rerank(query, documents) {
        ...
        const payload = await postJson(endpointUrl(this.#config.baseUrl, "rerank"), {
            model: this.#config.model,
            query,
            documents: normalized,
            top_n: normalized.length,
        }, this.#config, "resource routing reranker", this.#transport);
