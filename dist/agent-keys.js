 ...
 */
export function parseAgentKeysConf(text, sourceLabel) {
    ...
    for (const [index, rawLine] of lines.entries()) {
        ...
        if (separator < 0) {
            throw new Error(`${sourceLabel}:${lineNo}: expected "<agent> = <api key>", got ${JSON.stringify(rawLine)}`);
        }
        ...
        if (!AGENT_NAME_PATTERN.test(name)) {
            throw new Error(`${sourceLabel}:${lineNo}: agent name ${JSON.stringify(name)} must match [A-Za-z0-9_-]+ ` +
                "(it has to equal the OpenClaw agent id)");
        }
        if (name === SYSTEM_AGENT_ID) {
            throw new Error(`${sourceLabel}:${lineNo}: ${SYSTEM_AGENT_ID} is reserved; configure the system key as ` +
 ...
 */
function assertKeysAreDistinct(entries, systemApiKey, sourceLabel) {
    const seen = new Map();
    for (const [name, key] of entries) {
        const owner = seen.get(key);
        if (owner) {
            throw new Error(`${sourceLabel}: agents ${JSON.stringify(owner)} and ${JSON.stringify(name)} share the ` +
    ...
    if (systemOwner) {
        throw new Error(`${sourceLabel}: agent ${JSON.stringify(systemOwner)} uses the same API key as the system ` +
 ...
 */
export function loadAgentKeys(options) {
    const { logger, systemApiKey } = options;
    const filePath = options.filePath?.trim();
    const readFile = options.readFile ?? ((path) => readFileSync(path, "utf8"));
    const statMode = options.statMode ?? defaultStatMode;
    if (!filePath) {
        logger.warn("openviking: agentKeysFile is not configured — every agent will use the plugin apiKey and " +
    ...
    if (entries.size > 0 && !systemApiKey.trim()) {
        throw new Error(`openviking: agentKeysFile ${filePath} defines ${entries.size} agent key(s) but the plugin ` +
    ...
    if (mode !== undefined && (mode & 0o077) !== 0) {
        logger.warn(`openviking: agentKeysFile ${filePath} is readable beyond its owner ` +
            `(mode ${mode.toString(8).padStart(3, "0")}); chmod 600 it`);
    }
    const agentNames = [...entries.keys()].sort();
    logger.info(`openviking: per-agent credentials loaded from ${filePath} ` +
...
function systemOnlyResolver(systemApiKey) {
    return {
        agentNames: [],
        resolve: () => ({ apiKey: systemApiKey, account: SYSTEM_AGENT_ID, attributed: false }),
    };
}
function defaultStatMode(path) {
    try {
