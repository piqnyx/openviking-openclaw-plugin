...
export function createOpenVikingRecallTraceRuntime(deps) {
    ...
    const enrichTraceEntriesWithContent = async (result, includeContent, agentId, accountAgentId) => {
    ...
    const queryRecallTraces = async (input, session) => {
        ...
        return enrichTraceEntriesWithContent(base, shouldIncludeTraceContent(input), session.actorPeerId, session.agentId);
    };
