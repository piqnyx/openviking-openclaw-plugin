...
function isOpenVikingDirectoryRequiresRecursiveError(error) {
    return error instanceof Error &&
        error.message.includes("OpenViking request failed [FAILED_PRECONDITION]: Cannot remove directory without --recursive:");
}
...
export function registerOpenVikingImportTools(deps) {
    ...
    if (deps.enableRemoveResourceTool) {
        deps.registerTool((ctx) => ({
            ...
            description: "Use when the user explicitly asks to delete or remove one imported OpenViking resource/document. " +
            ...
            async execute(_toolCallId, params) {
                ...
                if (deps.resourceRouting?.enabled) {
                    ...
                    const categoryResolution = deps.resourceRouting.resolveCategoryOrFallback(session.agentId, selector);
                    const exactCurrentCategory = !categoryResolution.fallback ||
                        categoryResolution.fallbackReason === "organizational_category";
                    if (exactCurrentCategory) {
                        return rejectedResourceImport("Refusing to remove a taxonomy category/container. Remove a specific imported resource below that category instead.", { uri: validation.uri, protectedCategory: true });
                    }
                }
                const client = await deps.getClient(session.agentId);
                let result;
