    ...
    return withTimeout((async () => {
        ...
        const autoRecallPromises = searchPlan.searches.map(async (search) => {
            ...
            const result = await client.find(queryText, {
                ...
                // Память отбираем только по записям: L0/L1 — это описания каталогов,
                // на тематический запрос они обгоняют сами факты и съедают пул
                // кандидатов, который потом всё равно фильтруется до leaf ниже.
                // Ресурсы не трогаем: там уровни не фильтруются вовсе.
