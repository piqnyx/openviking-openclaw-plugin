    ...
    register(api) {
        ...
        try {
            agentKeys = loadAgentKeys({
                filePath: cfg.agentKeysFile,
                systemApiKey: cfg.apiKey,
                logger: api.logger,
            });
        }
        catch (keysErr) {
        ...
        if (cfg.peer_role !== "none") {
            api.logger.warn(`openviking: peer_role="${cfg.peer_role}" also splits every account into per-peer ` +
                'subtrees. With per-agent accounts that nesting is redundant — peer_role: "none" ' +
                "keeps each agent's file system flat.");
        }
